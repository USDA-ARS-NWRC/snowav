...
\begin{table}[h!]
	...
	\begin{tabular}{l c c  }
		...
		\multirow{2}{*} {\bf{Basin} }	& ppt (mean) & Rain \\ & [in] & [$\%$] \\
		
		\midrule
		Extended Tuolumne	& \VAR{TOTALPRE_PM|e} 	& $\sim$\VAR{TOTAL_RAT|e}  	\\
		Tuolumne	    			& \VAR{SUB1PRE_PM|e}		& $\sim$\VAR{SUB1_RAT|e} 	\\
		Cherry	   			& \VAR{SUB2PRE_PM|e}		& $\sim$\VAR{SUB2_RAT|e} 	\\
		Eleanor 	        		& \VAR{SUB3PRE_PM|e}		& $\sim$\VAR{SUB3_RAT|e} 	\\
		\bottomrule
	\end{tabular}
	% \caption{yupperdep}
	\label{tab:snotel2}
...
\begin{table}[h!]
	...
	\begin{tabular}{l c c c c c c }
		...
		\multirow{2}{*} {\bf{Basin} }	& SWE & SWE (avail) & SWE (mean) & $\Delta$SWE & SWI \\ & [\VAR{UNITS|e}] & [\VAR{UNITS|e}] & [in] & [\VAR{UNITS|e}]	& [\VAR{UNITS|e}]\\
		
		\midrule
		Extended Tuolumne	& \VAR{TOTAL_SWE|e} & \VAR{TOTAL_SWE_AV|e} & \VAR{TOTAL_PM|e} 	& \VAR{TOTAL_SWEDEL|e} & \VAR{TOTAL_SWI|e}  \\
		Tuolumne	    			& \VAR{SUB1_SWE|e}	& \VAR{SUB1_SWE_AV|e}  & \VAR{SUB1_PM|e} 	& \VAR{SUB1_SWEDEL|e} & \VAR{SUB1_SWI|e}		\\
		Cherry	   			& \VAR{SUB2_SWE|e}	& \VAR{SUB2_SWE_AV|e}  & \VAR{SUB2_PM|e} 	& \VAR{SUB2_SWEDEL|e} & \VAR{SUB2_SWI|e} 	\\
		Eleanor	        		& \VAR{SUB3_SWE|e}	& \VAR{SUB3_SWE_AV|e}  & \VAR{SUB3_PM|e} 	& \VAR{SUB3_SWEDEL|e} & \VAR{SUB3_SWI|e}	 	\\
		\bottomrule
	\end{tabular}
	\label{tab:snotel}
\end{table}
...
%		\toprule
%		
%		\multirow{2}{*} {\bf{Basin} }	& SWI  \\ & [\VAR{UNITS|e}]  \\
%		
%		\midrule
%		Extended Tuolumne	& \VAR{TOTAL_SWI|e}  \\
%		Tuolumne	    			& \VAR{SUB1_SWI|e}	 \\
%		Cherry	   			& \VAR{SUB2_SWI|e}	 \\
%		Eleanor 	        		& \VAR{SUB3_SWI|e}	 \\
%		\bottomrule
%	\end{tabular}
%	% \caption{yupperdep}
%	\label{tab:snotel2}
...
\begin{centering}
	...
	\caption{Change in SWE during the reporting period, as a depth (left) and as a function of elevation band (right). Since this is the first report of Water Year 2018, this is also the current model state of the snowpack.}
	\label{fig:RESULTS}
\end{centering}
\end{figure}
